...
int main(int argc, char **argv)
{
  ...
  while(ros::ok())
  {
    ...
    if (laser.isConnected()) 
    {
      ...
      //ROS_DEBUG("Laser configuration: scaningFrequency %d, angleResolution %d, startAngle %d, stopAngle %d", cfg.scaningFrequency, cfg.angleResolution, cfg.startAngle, cfg.stopAngle);
      //ROS_DEBUG("Laser output range:angleResolution %d, startAngle %d, stopAngle %d", outputRange.angleResolution, outputRange.startAngle, outputRange.stopAngle);
      ...
      scan_msg.angle_increment = 0.00872664619237;
      scan_msg.angle_min = -1.57079637051;
      ...
      scan_msg.time_increment = (outputRange.angleResolution / 10000.0) / 360.0 / (cfg.scaningFrequency / 100.0);

      ROS_DEBUG_STREAM("time increment : " << (double)scan_msg.time_increment << " seconds");

      scan_msg.ranges.resize(451);
      scan_msg.intensities.resize(451);
      ...
      while (ros::ok())
      {
        ...
        for (int i = 0; i < 451; i++)
        {
          scan_msg.ranges[i] = data.dist1[i+90] * 0.001;
        }

        for (int i = 0; i < 451; i++)
        {
          scan_msg.intensities[i] = data.rssi1[i+90];
        }
